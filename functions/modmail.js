...
const ChannelMessageHandler = async function (
    client,
    message,
    guild,
    messageUser,
    ...
    if (!currentGuildConfig.ticketParentId) {
        return message.reply(
            `${guild.name} has not set up init for Modmail, missing a channel category for tickets`,
        );
    }
    const UserTicketChannel = guild.channels.cache.find(
        (channel) => channel.name.includes(messageUser.id) == true,
    );
    ...
    if (UserTicketChannel) {
        const userEmbed = new Discord.MessageEmbed()
            ...
            .addFields({ name: 'User Message', value: `${message}` })
            .setTimestamp();
        // send message to open ticket
        await UserTicketChannel.send(userEmbed).catch((err) =>
            console.log(
                'There was a error with sending message to an open ticket in modmail',
                err,
            ),
        );
        return;
        ...
        try {
            const modmailEmbed = new Discord.MessageEmbed()
                ...
                .setDescription(
                    'We have sent your message to staff, expect a reply shortly. \nYou may continue to explain your issue here if needed.',
                )
        ...
        try {
            ...
            const userlogEmbed = new Discord.MessageEmbed()
                ...
                .setTitle('Staff Log')
                .setDescription(
                    `User history for ${messageUser}, ID: ${messageUser.id}`,
                )
                .setThumbnail(messageUser.avatarURL());
            if (tagList.length > 0) {
                tagList.forEach((t) => {
                    userlogEmbed.addFields({
                        name: `Log ID: ${t.id} | ${t.punishment} for ${t.reason}`,
                        value: `Done by Staff: ${t.staffName}`,
                    });
                });
            } else if (tagList.length === 0) {
                userlogEmbed.addField(
                    'Userlog',
            ...
            console.log(
                'There was an issue with the userlog Modmail function',
                e,
            );
        }
        const userEmbed = new Discord.MessageEmbed()
        ...
        await channel
            ...
            .catch((err) =>
                console.log(
                    'There was an error sending messages after creating channel in modmail',
                    err,
                ),
            );
    }
...
const ModmailGuildPrompt = async function (
    message,
    messageUser,
    memberGuildsArr,
) {
    ...
    const filter = (reaction, user) => {
        return (
            reactionArr.includes(reaction.emoji.name) &&
            user.id === messageUser.id
        );
    };
    const selectedGuild = await embedMessage
        .awaitReactions(filter, { time: 5000, errors: ['time'] })
        .catch((collected) => {
            ...
            if (!reaction) {
                message.channel.send(
                    `${messageUser.username}, you have timed out selecting a server. To try again please message here`,
                );
                return false;
            }
            const { guild } = memberGuildsArrReacts.find(
                (obj) => obj.reaction === reaction,
            );
            return guild;
        });
